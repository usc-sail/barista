...
class FasterOnlineDecoder {
 public:
  ...
  FasterOnlineDecoder(const fst::Fst<fst::StdArc> &fst,
                const FasterOnlineDecoderOptions &config);

  void SetOptions(const FasterOnlineDecoderOptions &config) { config_ = config; }
  
  ~FasterOnlineDecoder() { ClearToks(toks_.Clear()); }
 ...
 protected:
  ...
  FasterOnlineDecoderOptions config_;
